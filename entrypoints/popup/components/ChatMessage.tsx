...
export const ChatMessage: React.FC<ChatMessageProps> = ({ message, onCopy }) => {
  ...
  return (
    <div className={`flex gap-2 ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}>
      {message.type === 'ai' && (
        <div className="w-6 h-6 bg-black rounded-md flex items-center justify-center flex-shrink-0 mt-0.5">
      ...
      <div className={`max-w-[85%] ${message.type === 'user' ? 'order-first' : ''}`}>
        <div className={`
          px-3 py-2 rounded-md text-xs leading-relaxed
          ${message.type === 'user' 
        ...
        <div className={`flex items-center gap-1.5 mt-1 text-xs text-gray-500 ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}>
